...
function dedupeSongsById(songs = []) {
  ...
  for (const song of songs) {
    ...
    // force id to string (VERY important)
    const key = String(song.id);

    if (!seen.has(key)) {
      seen.set(key, { ...song, id: key });
    }
  }
...
useEffect(()=>{
  const clean = dedupeSongsById(suggestions);
  setSortableSuggestions(clean)
},[suggestions])
      ...
      {showSuggestions && (
        ...
        >
          ...
          {sortableSuggestions.length === 0 ? (
            ...
            >
              {sortableSuggestions.map((sugg,i) => (
                <div
                  key={sugg.id}
